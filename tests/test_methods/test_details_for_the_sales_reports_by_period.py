...
from wbapi_async.types import DetailsForTheSalesReportsByPeriodResponse


@pytest.mark.unit
class TestDetailsForTheSalesReportsByPeriod:
    async def test_details_for_the_sales_reports_by_period(self, api: MockedAPI) -> None:
        api.add_response(
            [
                {
                    "reportId": 1,
        ...
        result = await api.details_for_the_sales_reports_by_period(date_from="date_from", date_to="date_to")

        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], DetailsForTheSalesReportsByPeriodResponse)
