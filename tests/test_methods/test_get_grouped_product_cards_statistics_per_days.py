...
@pytest.mark.unit
class TestGetGroupedProductCardsStatisticsPerDays:
    async def test_get_grouped_product_cards_statistics_per_days(self, api: MockedAPI) -> None:
        api.add_response(
            {
                "data": [
                    {
        ...
        result = await api.get_grouped_product_cards_statistics_per_days(selected_period={})
