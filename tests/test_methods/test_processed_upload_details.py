...
@pytest.mark.unit
class TestProcessedUploadDetails:

    async def test_processed_upload_details(self, api: MockedAPI) -> None:
        api.add_response(
            {
            "data": {
            "historyGoods": [{
        ...
        result = await api.processed_upload_details(limit=1, upload_id=1)
