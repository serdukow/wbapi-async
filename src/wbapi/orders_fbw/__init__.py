...
class OrdersFbw:
    ...
    async def create_acceptance_option(
        ...
        """
        return await CreateAcceptanceOption(body=body, warehouse_id=warehouse_id).emit(self._api)
    ...
    async def get_supplies(
        ...
        *,
        ...
        """
        call = GetSupplies(dates=dates, limit=limit, offset=offset, status_ids=status_ids)
        return await call.paginate(self._api) if auto_paginate else await call.emit(self._api)

    async def iter_get_supplies(
        self,
        *,
        dates: list[ModelsDateFilterRequest] | None = None,
        limit: int | None = 1000,
        ...
        :param status_ids: Фильтр поставок по статусам. Возможные значения:   - `1` — Не запланировано   -
            `2` — Запланировано   - `3` — Отгрузка разрешена   - `4` — Идёт приёмка …
        """
        async for item in GetSupplies(dates=dates, limit=limit, offset=offset, status_ids=status_ids).stream(
            self._api
        ):
            yield item
    ...
    async def get_supplies_by_id(
        ...
        """
        return await GetSuppliesById(id_=id_, is_preorder_id=is_preorder_id).emit(self._api)

    async def get_supplies_goods(
        self,
        *,
        id_: str | int,
        ...
        :param auto_paginate: автоматически собрать все страницы выборки
        """
        call = GetSuppliesGoods(id_=id_, is_preorder_id=is_preorder_id, limit=limit, offset=offset)
        return await call.paginate(self._api) if auto_paginate else await call.emit(self._api)

    async def iter_get_supplies_goods(
        self,
        *,
        id_: str | int,
        is_preorder_id: bool | None = False,
        ...
        :param offset: После какого элемента выдавать данные
        """
        async for item in GetSuppliesGoods(
            id_=id_, is_preorder_id=is_preorder_id, limit=limit, offset=offset
        ).stream(self._api):
            yield item

    async def get_supplies_package(self, *, id_: str | int) -> list[ModelsBox]:
        """Упаковка поставки

        :param id_: ID поставки
        """
        return await GetSuppliesPackage(id_=id_).emit(self._api)

    async def get_transit_tariffs(self) -> list[ModelsTransitTariff]:
        """Транзитные направления"""
        return await GetTransitTariffs().emit(self._api)

    async def get_warehouses(self) -> list[ModelsWarehousesResultItems]:
        """Список складов"""
