...
class TariffsBox(WBMethod[RatesBoxResponse]):
    """Тарифы для коробов
...
class TariffsCommission(WBMethod[Fee]):
...
class TariffsPallet(WBMethod[RatesPalletResponse]):
...
class TariffsReturn(WBMethod[ReturnRatesResponse]):
    """Тарифы на возврат
...
class TariffsV1AcceptanceCoefficients(WBMethod[list[ModelsAcceptanceCoefficient]]):
    """Тарифы на поставку
