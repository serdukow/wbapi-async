...
class CommunicationsV2NewsResponse(WBModel):
    data: list[CommunicationsV2NewsResponseDataItem] | None = _field(default=None)
    """Новости"""


class CommunicationsV2NewsResponseDataItem(WBModel):
    ...
    """ID новости"""
    types: list[CommunicationsV2NewsResponseDataItemTypesItem] | None = _field(default=None)
    """Теги новости"""


class CommunicationsV2NewsResponseDataItemTypesItem(WBModel):
...
class InviteCreateAccessItem(WBModel):
    code: str | None = _field(default=None)
    """Код раздела профиля продавца, к которому пользователь получит доступ: * `balance` — Просмотр
    баланса и вывод средств * `brands` — Управление брендами …
    """
...
class InviteCreateInvite(WBModel):
    phone_number: str | None = _field(default=None, name="phoneNumber")
    """Номер телефона пользователя для приглашения. …"""
    position: str | None = _field(default=None)
    """Должность пользователя"""
...
class SellerInfoResponse(WBModel):
    name: str | None = _field(default=None)
