...
class AcceptanceOptionsCreate(WBMethod[ModelsOptionsResultModel]):
    """Опции приёмки
...
class Supplies(WBMethod[list[ModelsSupply]]):
    """Список поставок
...
class SuppliesId(WBMethod[ModelsSupplyDetails]):
    """Детали поставки
...
class SuppliesIdGoods(WBMethod[list[ModelsGoodInSupply]]):
    """Товары поставки
...
class SuppliesIdPackage(WBMethod[list[ModelsBox]]):
    """Упаковка поставки
...
class TransitTariffs(WBMethod[list[ModelsTransitTariff]]):
...
class Warehouses(WBMethod[list[ModelsWarehousesResultItems]]):
    """Список складов
