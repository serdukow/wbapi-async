...
from .methods import (
    Catalog,
    ContentAuthor,
    ContentAuthorContentId,
    ContentAuthorContentIdUpdate,
    ContentDelete,
    ContentDownloadUri,
    ContentGalleryCreate,
    ContentIllustrationCreate,
    ContentUploadChunkCreate,
    ContentUploadInitCreate,
    KeysApiKeysCreate,
    KeysApiKeysDelete,
    KeysApiKeysRedeemed,
    OfferKeysOfferId,
    OfferKeysOfferIdList,
    OfferOfferIdUpdate,
    OfferPriceOfferIdUpdate,
    OffersAuthor,
    OffersCreate,
    OffersOfferId,
    OffersOfferIdUpdate,
...
class Wbd:
    ...
    async def catalog(self) -> GetFullCatalogResponse:
        """Получить категории и их подкатегории"""
        return await Catalog().emit(self._api)

    async def content_author(
        self,
        *,
    ...
    async def iter_content_author(
        ...
        *,
        ...
        :param take: Количество контента для получения
        """
        async for item in ContentAuthor(
            category=category,
            search=search,
            skip=skip,
            sort=sort,
    ...
    async def content_author_content_id(self, *, content_id: str | int) -> Content:
        """Получить информацию о контенте

        :param content_id: ID контента
        """
        return await ContentAuthorContentId(content_id=content_id).emit(self._api)

    async def content_author_content_id_update(
        ...
        """
        return await ContentAuthorContentIdUpdate(
            content_id=content_id, description=description, title=title
        ).emit(self._api)

    async def content_delete(self, *, content_id: int | None = None) -> None:
        """Удалить контент

        :param content_id: ID контента
        """
        await ContentDelete(content_id=content_id).emit(self._api)

    async def content_download_uri(self, *, uri: str | int) -> None:
        """Скачать контент

        :param uri: URI-адрес контента
        """
        await ContentDownloadUri(uri=uri).emit(self._api)

    async def content_gallery_create(self) -> UploadGalleryResponse:
        """Загрузить медиафайлы для предложения"""
        return await ContentGalleryCreate().emit(self._api)

    async def content_illustration_create(self) -> IllustrationResponse:
        """Загрузить обложку контента"""
        return await ContentIllustrationCreate().emit(self._api)

    async def content_upload_chunk_create(self) -> UploadChunkResponse:
        """Загрузить контент (файл)"""
        return await ContentUploadChunkCreate().emit(self._api)

    async def content_upload_init_create(
        self,
        *,
        catalog_id: int,
        content_type: str,
        ...
        :param title: Название контента.Максимальная длина — **500 символов.**
        """
        return await ContentUploadInitCreate(
    ...
    async def keys_api_keys_create(self, *, keys: list[str], offer_id: int) -> None:
        ...
        """
        await KeysApiKeysCreate(keys=keys, offer_id=offer_id).emit(self._api)

    async def keys_api_keys_delete(self, *, ids: list[int]) -> KeysDeleteResponse:
        """Удалить ключи активации

        :param ids: Список ID ключей
        """
        return await KeysApiKeysDelete(ids=ids).emit(self._api)

    async def keys_api_keys_redeemed(
        self,
        *,
    ...
    async def iter_keys_api_keys_redeemed(
        ...
        *,
        ...
        :param take: Количество записей для получения. Указывает, сколько ключей должно быть возвращено в
            ответе
        """
        async for item in KeysApiKeysRedeemed(
            date_from=date_from, date_to=date_to, offer_id=offer_id, skip=skip, take=take
        ).stream(self._api):
            yield item

    async def offer_keys_offer_id(self, *, offer_id: str | int) -> KeysCountResponse:
        """Получить количество ключей для предложения

        :param offer_id: ID предложения
        """
        return await OfferKeysOfferId(offer_id=offer_id).emit(self._api)

    async def offer_keys_offer_id_list(
        self,
        *,
        offer_id: str | int,
    ...
    async def iter_offer_keys_offer_id_list(
        self,
        *,
        offer_id: str | int,
        deleted: bool | None = None,
        ...
        :param take: Количество записей для получения. Указывает, сколько ключей должно быть возвращено в
            ответе
        """
        async for item in OfferKeysOfferIdList(
            offer_id=offer_id,
            deleted=deleted,
            expired=expired,
            reserved=reserved,
    ...
    async def offer_offer_id_update(self, *, offer_id: str | int, status: int) -> None:
        """Обновить статус

        :param offer_id: ID предложения
        """
        await OfferOfferIdUpdate(offer_id=offer_id, status=status).emit(self._api)

    async def offer_price_offer_id_update(
    ...
    async def iter_offers_author(
        ...
        *,
        ...
        :param take: Количество предложений для получения
        """
        async for item in OffersAuthor(
            category=category,
            search=search,
            skip=skip,
            sort=sort,
    ...
    async def offers_create(self, *, body: Any) -> OfferResponse:
        """Создать новое предложение"""
        return await OffersCreate(body=body).emit(self._api)

    async def offers_offer_id(self, *, offer_id: str | int) -> OfferResponse:
        """Получить информацию о предложении

        :param offer_id: ID предложения
        """
        return await OffersOfferId(offer_id=offer_id).emit(self._api)

    async def offers_offer_id_update(
        self,
        *,
        offer_id: str | int,
    ...
    async def offers_thumb_create(self) -> None:
        """Добавить или обновить обложку предложения"""
        await OffersThumbCreate().emit(self._api)
