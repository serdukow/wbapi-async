...
def _latinise(x: str) -> str:
    ...
    return unicodedata.normalize("NFKD", x).encode("ascii", "ignore").decode() or _transliterate(x)
...
class DltSnakeCase:
    ...
    def normalize(cls, identifier: str) -> str:
        normalized = _latinise(identifier).strip().translate(cls._TR_REDUCE_ALPHABET)
        normalized = cls.RE_NON_ALPHANUMERIC.sub("_", normalized)
        return cls._to_snake_case(normalized)
    ...
    def _to_snake_case(cls, identifier: str) -> str:
        ...
        if cls.RE_LEADING_DIGITS.match(identifier):
            identifier = "_" + identifier

        stripped = identifier.rstrip("_")
        identifier = stripped + "x" * (len(identifier) - len(stripped))

        return cls.RE_UNDERSCORES.sub("_", identifier)


def field_name(x: str) -> str:
    x = DltSnakeCase.normalize(x).lstrip("_")
    if keyword.iskeyword(x):
        x += "_"
    return x or "field_"
...
# The banner every generated file opens with. "@generated" is the machine-
# readable half, understood by review tools; .gitattributes is what makes
# GitHub itself collapse these files in a diff.
BANNER = "# @generated\n# This file was auto-generated. Do not edit by hand.\n"


_NUMERIC_FORMAT = re.compile(r"^(u?int\d*|float\d*|double)$")


def _plural(count: int, single: str, few: str, many: str) -> str:
    """Russian picks a different form after 1, after 2-4, and after the rest."""
    tail, hundred = count % 10, count % 100
    if hundred in range(11, 15):
        return many
    if tail == 1:
        return single
    return few if tail in (2, 3, 4) else many
...
class Generator:
    ...
    def type_of(self, sch: dict | None, hint: str, depth: int = 0, response: bool = False) -> str:
        ...
        if "$ref" in sch:
            ...
            if not isinstance(target, dict) or not (
                ...
                return self.type_of(target, hint, depth + 1, response)
            name = pascal(sch["$ref"].split("/")[-1])
            if name not in self.structs:
                self.emit_struct(name, target, depth + 1, response)
            return name
        if isinstance(sch.get("allOf"), list) and sch["allOf"]:
            ...
            if merged is not None:
                return self.type_of(merged, hint, depth + 1, response)
        for combo in ("oneOf", "anyOf"):
            if combo in sch and isinstance(sch[combo], list) and sch[combo]:
                return self.type_of(sch[combo][0], hint, depth + 1, response)
        if sch.get("type") == "array":
            return f"list[{self.type_of(sch.get('items'), hint + 'Item', depth + 1, response)}]"
        if sch.get("type") == "object" or "properties" in sch:
            if not sch.get("properties"):
                return "dict[str, Any]"
            name = pascal(hint)
            if name not in self.structs:
                self.emit_struct(name, sch, depth + 1, response)
            return name
    ...
    def _description_of(self, sch: object) -> str:
        ...
        if isinstance(sch.get("allOf"), list):
            for branch in sch["allOf"]:
                ...
                if found:
                    return found
        return self._constraints_of(sch)

    def _constraints_of(self, sch: dict) -> str:
        """What the spec says about a field it left undescribed.

        A bare `orders` tells nothing, while "не более 100 элементов" is the
        difference between an FBS call and a DBS one.
        """
        limits = []
        if (most := sch.get("maxItems")) is not None:
            limits.append(f"не более {most} {_plural(most, 'элемент', 'элемента', 'элементов')}")
        if (least := sch.get("minItems")) is not None:
            limits.append(f"не менее {least} {_plural(least, 'элемент', 'элемента', 'элементов')}")
        if (longest := sch.get("maxLength")) is not None:
            limits.append(f"не длиннее {longest} {_plural(longest, 'символ', 'символа', 'символов')}")
        if (shortest := sch.get("minLength")) is not None:
            limits.append(f"не короче {shortest} {_plural(shortest, 'символ', 'символа', 'символов')}")
        if (highest := sch.get("maximum")) is not None:
            limits.append(f"не больше {highest}")
        if (lowest := sch.get("minimum")) is not None:
            limits.append(f"не меньше {lowest}")
        if (allowed := sch.get("enum")) is not None:
            limits.append("одно из: " + ", ".join(f"`{value}`" for value in allowed[:8]))
        # A numeric format only restates the type, but a date format says how
        # the value has to be written.
        shape = sch.get("format")
        if isinstance(shape, str) and not _NUMERIC_FORMAT.match(shape):
            limits.append(f"формат {shape}")
        return ", ".join(limits).capitalize()

    def emit_struct(self, name: str, sch: dict, depth: int = 0, response: bool = False) -> None:
        ...
        for prop, ps in sorted(props.items()):
            field = field_name(prop) if response else snake(prop)
            while field in used:
                field += "_"
            used.add(field)
            pytype = self.type_of(ps, f"{name}{pascal(prop)}", depth + 1, response)
    ...
    def _method(
        ...
        body_sch = (request_body.get("content") or {}).get("application/json", {}).get("schema")
        ...
        return dict(  # noqa: C408 — named fields read better than a literal
            ...
            return_type=(
                self.type_of(rsch, f"{pascal(name)}Response", response=True)
                if rsch
                else ("bytes" if binary else "None")
            ),
        )
    ...
    def render_methods(self) -> str:
        ...
        for m in sorted(self.methods, key=lambda x: x["name"]):
            ...
            if m["body_required"]:
                out.append("    __body_required__ = True")
            if fields:
                out.append("")
                out.extend(fields)
            out.append("")
...
def section_heading(package: str) -> str:
    """Latin only: a Cyrillic anchor is percent-encoded by some renderers and
    left alone by others, so the link resolves in one and not the next. The
    Russian title goes on the line below the heading."""
    return f"api.{package}"


def heading_anchor(heading: str) -> str:
    """The anchor GitHub derives from a heading: punctuation dropped, spaces
    hyphenated, the rest lowercased."""
    slug = re.sub(r"[^\w\s-]", "", heading.lower().strip(), flags=re.UNICODE)
    return re.sub(r"\s+", "-", slug)


def render_method_index(collected: dict[str, list[dict]]) -> str:
    """A table of every generated method, so a name can be found without grep.

    The heading carries the package name as well as the Russian title: GitHub
    builds an anchor out of the heading text, and a link to #analytics would
    not reach a heading that reads "Аналитика".
    """
    total = sum(len(rows) for rows in collected.values())
    out = [
        "# Методы",
        "",
        f"{total} методов в {len(collected)} разделах.",
        "",
    ...
    for package in collected:
        title = SECTION_TITLES.get(package, package)
        out.append(f"| [{title}](#{heading_anchor(section_heading(package))}) | {len(collected[package])} |")

    for package, rows in collected.items():
        title = SECTION_TITLES.get(package, package)
        out += [
            "",
            f"## {section_heading(package)}",
            "",
            f"{title} — {len(rows)} методов.",
            "",
            "| Метод | HTTP | Эндпоинт | Описание |",
            "| --- | --- | --- | --- |",
        ]
        for meth in sorted(rows, key=lambda m: m["name"]):
            summary = safe_doc(meth["summary"]).replace("|", "\\|")
            out.append(f"| `{meth['name']}()` | {meth['verb']} | `{meth['path']}` | {summary} |")
    return "\n".join(out) + "\n"


def generate_section(spec_file: pathlib.Path, package: str) -> tuple[int, int, str, list[dict]]:
    ...
    return structs, len(generator.methods), generator.title, generator.methods
...
def main() -> int:
    ...
    for spec_name, package in SECTIONS.items():
        ...
        if not spec_file.exists():
            ...
            continue
        try:
            models, methods, title, collected = generate_section(spec_file, package)
        ...
        by_section[package] = collected
        total_models += models
        total_methods += methods
        print(f"  {package:18} {methods:4d} methods, {models:4d} models   {title}")

    # Only when every section was rebuilt: a partial run would drop the rest.
    if not only and not failed:
        (ROOT / "METHODS.md").write_text(render_method_index(by_section))
